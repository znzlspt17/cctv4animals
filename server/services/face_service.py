...
class FaceService:
            ...
            self._embedding_cache.append(
                {
                    ...
                    "person_name": person_map.get(r["person_id"], "Unknown"),
    ...
    def search_face(self, image_bytes: bytes, repo) -> list:
        ...
        # D2, D3: 품질 필터
        valid_faces = self.validate_detection_frame(faces)
        if not valid_faces:
            return []

        # 캐시가 비어 있으면 모두 Unknown
        results = []
        for face in valid_faces:
            ...
            # 임베딩 추출
            embedding_np = self._extract_embedding_from_face(face, img)
            if embedding_np is None:
                continue
